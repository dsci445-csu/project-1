...
df <- df %>% select(-id, -Profession, -`Work Pressure`, -`Job Satisfaction`)
...
k_grid <- tibble(neighbors = seq(1, 27, by = 2))
...
show_best(tuned, metric = "accuracy")
...
metrics <- predictions %>% metrics(truth = Depression, estimate = .pred_class)
conf_mat <- predictions %>% conf_mat(truth = Depression, estimate = .pred_class)

metrics
conf_mat

cv_results <- tuned %>% collect_metrics()
cv_results %>%
  ...
  geom_point() +
  labs(
    title = "KNN Cross-Validated Accuracy vs k",
