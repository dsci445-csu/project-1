...
# 1. Apply the final trained workflow to the test data
test_predictions <- predict(final_fit_results, new_data = test_data) |>
  bind_cols(predict(final_fit_results, new_data = test_data, type = "prob")) |>
  # 2. Add the true outcome variable for comparison
  bind_cols(test_data |> select(Depression))

# 3. Generate the Confusion Matrix
conf_matrix <- conf_mat(test_predictions, truth = Depression, estimate = .pred_class)

cat("## Confusion Matrix for LASSO Logistic Regression\n")
print(conf_matrix)

# 4. Calculate ALL Summary Classification Metrics
metrics_summary <- conf_matrix |>
  summary()

# 5. Extract and print the overall Accuracy
accuracy_value <- metrics_summary |>
  dplyr::filter(.metric == "accuracy") |>
  dplyr::pull(.estimate)

cat("\nTest set accuracy:", round(accuracy_value, 4), "\n")


# 6. Optional: Print other key metrics
cat("\n## Other Key Classification Metrics\n")
metrics_to_show <- metrics_summary |>
  dplyr::filter(.metric %in% c("sensitivity", "specificity", "precision", "roc_auc")) |>
...
# We use the previously calculated estimate of ~67 total features.
total_predictors <- 67
